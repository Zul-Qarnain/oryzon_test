...
// Helper to format service responses into a structured string
function formatObjectToString(obj: any, title: string): string {
  ...
  const formatValue = (value: any, indentLevel = 1): string => {
    ...
    if (Array.isArray(value)) {
      ...
      subResult += keys.map(k => `${indent}  "${k}": ${formatValue(value[k], indentLevel + 1).trimStart()}`).join(',\n');
      subResult += `\n${indent}}`;
      return subResult;
...
export const getAITools = (customerId: string, connectedPageID: string) => {
  const tools = {
    // --- Product Tools ---
    getProductById: tool({
      description: 'Get detailed information about a specific product by its ID. Ask the user for the Product ID.',
      parameters: z.object({
        productId: z.string().describe('The unique ID of the product to retrieve.'),
      }),
      execute: async ({ productId }) => {
        const product = await productsService.getProductById(productId, { include: { user: false, connectedChannel: true } });
        
        // Product must be associated with the current channel.
        if (!product || product.channelId !== connectedPageID) {
          // Also, ensure the user derived from the channel owns the product for integrity, though channelId is primary.
          const userIdFromChannel = await getUserIdFromChannel(connectedPageID);
          if (!product || product.userId !== userIdFromChannel) {
    ...
    getProductByImageUrl: tool({
      ...
      execute: async ({ imageUrl }) => {
        ...
        const channelUser = await getUserIdFromChannel(connectedPageID);
        if (!channelUser) return formatObjectToString({ error: 'Channel context is invalid.' }, 'Product Information');
        ...
        if (!productsResult.data.length || productsResult.data[0].userId !== channelUser) {
    ...
    createProduct: tool({
      description: `Create a new product, associating it with the current channel. Ask the user for the following details:
        ...
        - metadata (JSON object, optional): Any additional structured data.`,
      parameters: z.object({
        ...
        metadata: z.record(z.unknown()).optional().describe('Any additional structured data as a JSON object.'),
      }),
      execute: async (productParams) => {
        ...
        const productData: CreateProductData = {
          ...
          channelId: connectedPageID, // Associate with the current channel
        };
    ...
    updateProductInfo: tool({
      description: `Update an existing product associated with the current channel. Ask the user for the Product ID and the fields they want to update from the following:
        ...
        - metadata (JSON object): New additional structured data.
        Note: This tool only updates products within the current channel context.`,
      parameters: z.object({
        ...
        metadata: z.record(z.unknown()).optional().describe('New additional structured data as a JSON object.'),
      }),
